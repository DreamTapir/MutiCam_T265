...
void main(void){
    ...
    if(age >= maxAge){
        ...
        vel.xyz = vec3(normalize(startPos) * 5);
    }
    
    // Curl Noiseで速度を更新
    vel.x += snoise(vec4(pos.x * u_scale, pos.y * u_scale, pos.z * u_scale, 0.1352 * u_time * u_timestep));
    vel.y += snoise(vec4(pos.x * u_scale, pos.y * u_scale, pos.z * u_scale, 0.2814 * u_time * u_timestep));
    vel.z += snoise(vec4(pos.x * u_scale, pos.y * u_scale, pos.z * u_scale, 0.5564 * u_time * u_timestep));
